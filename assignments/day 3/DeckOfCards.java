...
class Card implements Comparable<Card> {
    ...
    Card(String s,String r,int a,int b) {
     ...
     public int compareTo(Card c) {
         ...
         if (this.Intsuit < c.Intsuit) {
             return 1;
        }
        else if (this.Intsuit > c.Intsuit) {
           return -1;
        } 
        else {
           if (this.Intrank < c.Intrank) {
               return 1;
           }
           else if (this.Intrank > c.Intrank) {
           return -1;
           }
           else {
                return 0;
   ...
   Card[] cardArray() {
    ...
    void toString(Card[] cards) {
        for(int i = 0; i < size; i++) {
            System.out.println(cards[i].suit + " " + cards[i].rank + " " + cards[i].Intsuit + " " + cards[i].Intrank);
        }
    }
